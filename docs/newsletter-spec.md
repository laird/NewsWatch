...
- Present newsletter in 'old time' newspaper style, with a banner, stories, and a footer.
  - Banner should be NEWSWATCH, with the date/time on the left, and 'price' on the right.
    ...
    - the banner title should be NEWSWATCH. If the env is dev, append (dev) to the banner.
    - The 'cost' should be the number of tokens consumed to generate the newsletter. This means that all processing should capture token use.
